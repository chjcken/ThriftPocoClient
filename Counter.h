...
class CounterIf {
 public:
  ...
  virtual int get(const std::string& id) = 0;
  virtual void inc(const std::string& id, const int num) = 0;
};
...
class CounterNull : virtual public CounterIf {
 public:
  ...
  void inc(const std::string& /* id */, const int /* num */) {
    return;
  }
};
...
typedef struct _Counter_get_args__isset {
  _Counter_get_args__isset() : id(false) {}
  bool id :1;
...
class CounterClient : virtual public CounterIf {
 ...
 public:
  ...
  int recv_getTotal();
  int get(const std::string& id);
  void send_get(const std::string& id);
  int recv_get();
  void inc(const std::string& id, const int num);
